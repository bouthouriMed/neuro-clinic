...
export default function PublicNavbar() {
  ...
  // On the homepage, before scrolling: transparent with light text
  // After scrolling (or on other pages): white bg with dark text
        ...
        <div className="h-20 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 no-underline group">
            <div className="relative">
              ...
              <div className={`absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 ${isTransparent ? 'border-transparent' : 'border-white'}`} />
            </div>
            <div className="flex flex-col">
              <span className={`text-base font-bold tracking-tight transition-colors duration-500 ${isTransparent ? 'text-white' : 'text-slate-900'}`}>NeuroClinic</span>
              <span className={`text-[10px] font-medium tracking-wide uppercase transition-colors duration-500 ${isTransparent ? 'text-white/60' : 'text-slate-400'}`}>Dr. Abir Bouthouri</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link to="/dashboard" className="no-underline">
              <Button variant="ghost" size="sm" className={`transition-colors duration-500 ${isTransparent ? 'text-white/70 hover:text-white hover:bg-white/10' : 'text-slate-500 hover:text-indigo-600'}`}>
                Espace Medecin
              </Button>
            </Link>

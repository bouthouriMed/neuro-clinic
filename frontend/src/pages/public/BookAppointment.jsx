...
export default function BookAppointment() {
  ...
  const hasValidContact = () => form.phone || (form.email && isValidEmail(form.email))
  ...
  const successIconRef = useCallback((node) => {
    if (node) {
      // Wait for layout to settle, then scroll icon into view below the fixed navbar
      setTimeout(() => {
        const navbar = document.querySelector('nav')
        const navbarHeight = navbar ? navbar.offsetHeight : 0
        const top = node.getBoundingClientRect().top + window.scrollY - navbarHeight - 16
  ...
  useEffect(() => {
    ...
    if (oauth === 'success') {
      ...
      if (email) {
        ...
        // Scroll to confirm button after OAuth redirect
        setTimeout(() => {
  ...
  return (
    <>
      ...
      {/* Form */}
      <section className={`${step === 2 ? 'py-6' : 'py-12'} bg-gradient-to-br from-slate-50 via-white to-indigo-50 relative overflow-x-hidden`}>
        ...
        <div className="max-w-3xl mx-auto px-6 relative">
          ...
          {step === 0 && (
            <div className="space-y-8">
              <div ref={timeSectionRef} className="scroll-mt-4 bg-white rounded-3xl p-6 shadow-xl border border-slate-100">
              ...
              {form.date && (
                <div ref={continueButtonRef} className="scroll-mt-4 bg-white rounded-3xl p-6 shadow-xl border border-slate-100 animate-fade-in-up">
                  <h3 className="text-lg font-bold text-slate-800 mb-1 flex items-center gap-2">
          ...
          {/* Step 2: User Info */}
          {step === 1 && (
            <div ref={step2Ref} className="scroll-mt-4 space-y-6 animate-fade-in-up">
              <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100">
                ...
                {/* OAuth */}
                <div className="mb-6">
                  <p className="text-sm font-medium text-slate-600 mb-3 text-center">Remplir rapidement avec</p>
                  <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => handleOAuth('google')} className="flex items-center justify-center gap-2 px-4 py-3 bg-white border-2 border-slate-200 rounded-xl hover:border-slate-300 hover:shadow-md active:scale-95 transition-all cursor-pointer">
                      ...
                      <span className="text-sm font-medium text-slate-700 hidden sm:inline">Google</span>
                    </button>
                    <button onClick={() => handleOAuth('facebook')} className="flex items-center justify-center gap-2 px-4 py-3 bg-[#1877F2] border-2 border-[#1877F2] rounded-xl hover:bg-[#1565D8] hover:shadow-md active:scale-95 transition-all cursor-pointer">
                      <svg className="w-5 h-5" fill="white" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                      <span className="text-sm font-medium text-white hidden sm:inline">Facebook</span>
                    </button>
                    <button onClick={() => handleOAuth('instagram')} className="flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#F77737] border-2 border-transparent rounded-xl hover:shadow-md active:scale-95 transition-all cursor-pointer">
                      ...
                      <span className="text-sm font-medium text-white hidden sm:inline">Instagram</span>
                    </button>
                  </div>
                  <div className="flex items-center gap-3 mt-4">
                    <div className="flex-1 h-px bg-slate-200" />
                    <span className="text-xs text-slate-400 font-medium">ou remplissez manuellement</span>
                    <div className="flex-1 h-px bg-slate-200" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                  ...
                  <Input label="Téléphone" icon={Phone} type="tel" inputMode="tel" placeholder="+216 XX XXX XXX" value={form.phone} onChange={(e) => { update('phone', e.target.value); setContactError('') }} error={contactError && !form.email ? contactError : ''} />
                </div>
                <div className="mt-5">
                  <Input 
                    label="Email" 
                    ...
                    required 
                  />
                  <p className="text-xs text-slate-400 ml-1 mt-1">* Email ou téléphone requis pour la confirmation</p>
                </div>
                <div className="mt-5">
                ...
                {!oauthUser && (
                  <div className="mt-5 flex items-center gap-3">
                    ...
                    />
                    <label htmlFor="createAccount" className="text-sm text-slate-600 cursor-pointer">
                      Créer un compte patient pour gérer mes rendez-vous
                    </label>
                  </div>
                )}
              ...
              <div ref={confirmButtonRef} className="flex justify-between gap-4">
                <Button variant="secondary" onClick={() => setStep(0)} className="px-5">
                  <ArrowLeft className="w-4 h-4 mr-2" /> Retour
                </Button>
                <Button onClick={() => {
                  if (!hasValidContact()) {
                    setContactError('Veuillez fournir un email ou un numéro de téléphone')
                    return
                  }
                  setContactError('')
                  handleSubmit()
                }} disabled={!form.name || !hasValidContact() || loading} size="lg" variant="primary" className="px-8">
          ...
          {/* Step 3: Success */}
          {step === 2 && success && (
            <div ref={step3Ref} className="max-w-sm mx-auto">
              {/* Success header */}
              <div ref={successIconRef} className="text-center mb-6">
                ...
                <p className="text-slate-500 text-sm mt-1">📧 Un email de confirmation vous a été envoyé.</p>
              </div>
              ...
              <div className="bg-white rounded-3xl p-4 shadow-xl border border-slate-100 mb-4 overflow-hidden">
                ...
                <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(doctor.contact.address)}`} target="_blank" rel="noopener" className="mt-2 inline-flex items-center justify-center gap-1.5 w-full py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 transition-colors">
